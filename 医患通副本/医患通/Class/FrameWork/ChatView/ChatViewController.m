...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, IDeviceManagerDelegate,EMCDDeviceManagerDelegate>
{
...
- (void)setupBarButtonItem
{
    ...
    [backButton setImageEdgeInsets:UIEdgeInsetsMake(0, -18, 0, 0)];
    ...
    [clearButton setImageEdgeInsets:UIEdgeInsetsMake(0, 0, 0, -18)];
    ...
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (cell == nil) {
                ...
                if (model.isSender) {
                    cell.headImageView.image = [UIImage imageNamed:@"chatListCellHead"];
                }
                else{
                    cell.headImageView.image = [UIImage imageNamed:@"doctor.jpg"];
                }
            }
...
- (void)removeAllMessages:(id)sender
{
    if (_dataSource.count == 0) {
        [self showHint:NSLocalizedString(@"没有删除的信息", @"no messages")];
        return;
    }
